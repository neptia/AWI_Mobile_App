...
struct GameSearchView: View {
    @StateObject private var viewModel = SearchGamesViewModel()
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                .searchSuggestions {
                    Section {
                        // Filtered games suggestions, displaying up to 5 results
                        ForEach(viewModel.filteredGames(searchText: searchText).prefix(5), id: \.id) { suggestion in
