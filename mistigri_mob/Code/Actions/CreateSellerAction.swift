...
struct CreateSellerAction {
    var parameters: SellerCreationRequest
    func call(onSuccess: @escaping (SellerCreationResponse) -> Void, onError: @escaping (String) -> Void) {
        let path: String = "/sellers"
        ...
        let task = URLSession.shared.dataTask(with: request) { data, testResponse, error in
            ...
            if let data = data {
                ...
                let response = try? JSONDecoder().decode(SellerCreationResponse.self, from: data)
